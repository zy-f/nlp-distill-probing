...
def main():
    ...
    train_data = DistilBERTLikeDataset(mnli_tok['train'])
    ...
    with SummaryWriter(log_dir=log_dir) as log_writer:
        for epoch in range(n_epochs):
            tr_loss, tr_pred, tr_y = distilbert_epoch_loop(model, train_dl, loss_func, device=device, optimizer=optimizer)
            ...
            with torch.no_grad():
                v_m_loss, v_m_pred, v_m_y = distilbert_epoch_loop(model, val_m_dl, loss_func, device=device)
